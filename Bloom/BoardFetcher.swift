...
public class BoardFetcher: ObservableObject {
    @Published var fetchedBoards = [FetchedBoard]()
    ...
    func load() {
        ...
        URLSession.shared.dataTask(with: url) {(data,response,error) in
            do {
                if let d = data {
                    let decodedLists = try JSONDecoder().decode(FetchedBoards.self, from: d)
                    DispatchQueue.main.async {
                        
                        self.fetchedBoards = decodedLists.boards
                    }
                } else {
                    print("No Data")
                }
...
//
//   let fetchedBoards = try? newJSONDecoder().decode(FetchedBoards.self, from: jsonData)


// MARK: - FetchedBoards
struct FetchedBoards: Codable {
...
// MARK: - Board
struct FetchedBoard: Codable, Identifiable {
    let id = UUID()
